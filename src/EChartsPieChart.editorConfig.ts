    ...
    values.series?.forEach((s: any, i: number) => {
        if (s.dataSet === "static") {
            hideInObject(defaultProperties, "series", i, [
                "dynamicDataSource", "groupByAttribute",
                "dynamicNameAttribute", "dynamicValueAttribute",
                "dynamicTooltipHoverText", "dynamicSliceColor", "dynamicOnClickAction"
            ]);
        } else {
            hideInObject(defaultProperties, "series", i, [
                "staticDataSource", "staticName",
                "staticNameAttribute", "staticValueAttribute",
                "staticTooltipHoverText", "staticSliceColor", "staticOnClickAction"
            ]);
        }
    });

    if (!values.donut) {
        hideProperties(defaultProperties, ["innerRadius"]);
    }
