...
export declare interface ActionValue<TArguments extends ActionVariableArguments | "none" = "none"> {
    ...
    /**
     * The address of the page associated with the action, if any.
     *
     * @example
     * { status: "available", value: { href: "/p/pageName" } }
     *
     * @example
     * // Mendix pages with page parameters are supported
     * { status: "available", value: { href: "/p/pets/dogs/Barry" } }
     *
     * @example
     * { status: "available", value: { href: "https://www.mendix.com", target: "_blank" } }
     *
     * @example
     * { status: "unavailable", value: undefined }
     *
     * @example
     * // Loading may keep the previous value or be undefined
     * { status: "loading", value: { href: "/p/previous", target: "_self" } }
     *
     * @remark
     * This property is optional and may be undefined if no address is associated with the action.
     * This is only applicable for ShowPage actions to pages with a URL and openLink actions with a literal address value.
     */
    readonly address?: DynamicValue<{
        href?: string;
        target?: "_self" | "_blank";
    }>;
}
...
declare interface EditableAttributeValueBase<T extends AttributeValue_2 = AttributeValue_2> extends EditableValueBase<T> {
    /**
...
/**
 * Prop value representing an editable file value with validation. Like {@link DynamicValue}, this
 * value has a {@link ValueStatus}. Additionally, the value may be in the {@link readOnly} state which prevents editing.
 **/
export declare interface EditableFileValue<T = FileValue> extends EditableValueBase<File> {
    /**
     * The current value.
     *
     * @remark
     * To set this value use {@link setValue}, if the property is not read-only.
     */
    readonly value: Option_2<T>;
}

/**
 * Prop value representing an editable image file value with validation. Like {@link DynamicValue}, this
 * value has a {@link ValueStatus}. Additionally, the value may be in the {@link readOnly} state which prevents editing.
 *
 * @typeParam T The type of the contained value, which must be a subtype of {@link ImageValue}.
 **/
export declare interface EditableImageValue<T extends ImageValue> extends EditableFileValue<T> {
    /**
     * Sets the thumbnail size directly, the size will only be applied when uploading the image file.
     *
     * @param width The thumbnail width value.
     * @param height The thumbnail width value.
     */
    setThumbnailSize: (width: Option_2<number>, height: Option_2<number>) => void;
}
 ...
 * @beta
 */
export declare interface EditableListValue<T extends AttributeValue_2 = AttributeValue_2> extends EditableAttributeValueBase<T> {
    /**
 ...
 * @typeParam T The type of the contained value, which must be a subtype of {@link AttributeValue}.
 */
export declare interface EditableValue<T extends AttributeValue_2 = AttributeValue_2> extends EditableAttributeValueBase<T> {
    /**
     * The value, if available, or the previous value when currently loading.
     */
    readonly value: Option_2<T>;
...
declare interface EditableValueBase<T> {
    /**
